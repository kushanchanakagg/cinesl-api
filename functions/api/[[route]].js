...
function rewriteM3u8(body, url, extraParam = '', sourceKey = null) {
    const base = url.split('?')[0];
    const dir = base.slice(0, base.lastIndexOf('/') + 1);
    const origin = new URL(url).origin;
    
    // Get source headers if sourceKey provided
    let headersParam = '';
    if (sourceKey) {
        const mod = SOURCE_MODULES[sourceKey];
        const headers = mod?.HEADERS || mod?.VERIFY_HEADERS || {};
        headersParam = '&headers=' + encodeURIComponent(JSON.stringify(headers));
    }
    ...
    return body.split('\n').map(line => {
        ...
        if (t.startsWith('#')) {
            return t.replace(/URI="([^"]+)"/g, (match, uri) => {
                ...
                return `URI="https://cdn.cinesl.top/proxy.php?url=${encodeURIComponent(abs)}${extraParam}${headersParam}"`;
            });
        }
        const abs = t.startsWith('http') ? t : t.startsWith('/') ? origin + t : dir + t;
        if (abs.includes('tiktokcdn.com') || abs.includes('p16-sg') || abs.includes('p19-sg')) return `https://cdn.cinesl.top/proxy.php?url=${encodeURIComponent(abs)}&tt=1${headersParam}`;
        return `https://cdn.cinesl.top/proxy.php?url=${encodeURIComponent(abs)}${extraParam}${headersParam}`;
    }).join('\n');
}
...
function wrapUrl(rawUrl, sourceKey) {
    ...
    // Convert headers to URL parameter
    const headersParam = encodeURIComponent(JSON.stringify(headers));
    
    return `https://cdn.cinesl.top/proxy.php?url=${encodeURIComponent(raw)}&${cfg.proxyParam}=1&headers=${headersParam}`;
}
                    ...
                    try {
                        ...
                        if (looksLikeM3u8) {
                            ...
                            if (text.trim().startsWith('#EXTM3U')) {
                                const rewritten = rewriteM3u8(text, rawUrl, `&${cfg.proxyParam}=1`, reqUrl.origin, matchedSource.key);
